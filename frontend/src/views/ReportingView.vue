  ...
  axios.get(url)
      .then(response => {
        if (response.status === 200 && (!response.data || Object.keys(response.data).length === 0)) {
          error.value = `What a sad day! There were no sales on ${when.value}`;
        }
...
<template>
  ...
  <Layout>
    <div class="flex-grow flex flex-col gap-10 justify-center items-center">
      <form action="#" @submit.prevent="handleSubmit">
        <div>
          <input id="date" v-model="when" class="border p-2.5 rounded-md" name="date" type="date"
                 @change="handleSubmit">
        </div>
      </form>
